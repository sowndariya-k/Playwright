...
- Name: task\task2.test.ts >> Register User
- Location: tests\task\task2.test.ts:3:1
...
```ts
  ...
  4  | 
  5  |     await page.goto(
  6  |         'https://www.testmuai.com/selenium-playground/simple-form-demo/'
  7  |     );
  8  | 
  9  |     await page.locator('#sum1').fill('12');
  10 |     await page.locator('#sum2').fill('12');
  11 | 
  12 |     console.log("Sum1:", await page.locator('#sum1').inputValue());
  13 |     console.log("Sum2:", await page.locator('#sum2').inputValue());
  14 | 
  15 | 
  16 |     const getSumButton = page.locator(
  17 |         'button:has-text("Get Sum")'
  18 |     );
  19 | 
  20 |     await expect(getSumButton).toBeVisible();
  21 | 
  22 |     await getSumButton.click();
  23 | 
  24 | 
  25 |     await page.waitForTimeout(1000);
  26 | 
  27 |     console.log(
  28 |         "Message after click:",
  29 |         await page.locator('#addmessage').textContent()
  30 |     );
  31 | 
  32 | 
  33 |     await expect(page.locator('#addmessage'))
> 34 |         .toHaveText('24');
     |          ^ Error: expect(locator).toHaveText(expected) failed
  35 | 
  36 | });
