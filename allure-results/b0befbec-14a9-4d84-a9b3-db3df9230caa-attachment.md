...
Call log:
  - waiting for getByRole('radio', { name: 'Chrome' }).first()
  ...
  - generic [ref=e2]:
    - generic [ref=e3]:
      - link [ref=e4] [cursor=pointer]:
        - /url: /dashboard.xhtml;jsessionid=node039m3eki57vrundivygryernf19728040.node0
      - text: 
...
```ts
  ...
  6  | 
  7  |     // Select the first "Chrome" radio button
  8  |     const chrome = page.getByRole("radio", { name: "Chrome" }).first();
  9  | 
> 10 |     await chrome.check();
     |                  ^ Error: locator.check: Test timeout of 30000ms exceeded.
  11 | 
  12 |     await expect(chrome).toBeChecked();
  13 | 
  14 |     console.log("Chrome selected");
  15 | 
  16 | });
```