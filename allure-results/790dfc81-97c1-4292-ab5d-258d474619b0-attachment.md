...
Locator: locator('.ui-radiobutton-box').first()
Expected pattern: /ui-state-active/
Received string:  "ui-radiobutton-box ui-widget ui-corner-all ui-state-default"
Timeout: 5000ms

Call log:
  - Expect "toHaveClass" with timeout 5000ms
  - waiting for locator('.ui-radiobutton-box').first()
    14 × locator resolved to <div class="ui-radiobutton-box ui-widget ui-corner-all ui-state-default">…</div>
       - unexpected value "ui-radiobutton-box ui-widget ui-corner-all ui-state-default"
  ...
  - generic [ref=e2]:
    - generic [ref=e3]:
      - link [ref=e4] [cursor=pointer]:
        - /url: /dashboard.xhtml;jsessionid=node016miptdwx2ec51w8yareknwu9c19728099.node0
      - text: 
...
```ts
  ...
  6  | 
  7  |     // Click the visible Chrome label
  8  |     await page.locator("//label[normalize-space()='Chrome']").first().click();
  9  | 
  10 |     // Verify the visible radio button is active
  11 |     const radioBox = page.locator(".ui-radiobutton-box").first();
  12 | 
> 13 |     await expect(radioBox).toHaveClass(/ui-state-active/);
     |                            ^ Error: expect(locator).toHaveClass(expected) failed
  14 | 
