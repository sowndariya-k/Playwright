...
- Location: tests\PracticeActions\checkbox.test.ts:57:1

# Error details

```
Error: locator.click: Error: strict mode violation: getByText('Paris', { exact: true }) resolved to 2 elements:
    1) <label for="j_idt87:multiple:2">Paris</label> aka locator('[id="j_idt87:multiple"]').getByText('Paris')
    2) <label for="57fbff18-a8ef-42a2-91fe-be82909f4219">Paris</label> aka locator('[id="j_idt87:multiple_panel"]').getByText('Paris')

Call log:
  - waiting for getByText('Paris', { exact: true })
  ...
  - generic [ref=e2]:
    - generic [ref=e3]:
      - link [ref=e4] [cursor=pointer]:
        - /url: /dashboard.xhtml;jsessionid=node01128rgcznidqbipca2aapl4pp19727699.node0
      - text: 
...
```ts
  ...
  58 | 
  59 |     await page.goto("https://leafground.com/checkbox.xhtml");
  60 | 
  61 |     await page.locator(".ui-icon-triangle-1-s").click();
  62 | 
> 63 |     await page.getByText("Paris", { exact: true }).click();
     |                                                    ^ Error: locator.click: Error: strict mode violation: getByText('Paris', { exact: true }) resolved to 2 elements:
  64 | 
  65 |     await page.getByText("London", { exact: true }).click();
  66 | 
  67 |     await page.keyboard.press("Escape");
  68 | 
  69 |     await expect(page.locator(".ui-selectcheckboxmenu-label"))
  70 |         .toContainText("Paris");
  71 | 
  72 |     await expect(page.locator(".ui-selectcheckboxmenu-label"))
  73 |         .toContainText("London");
  74 | 
  75 | });
```