...
```ts
  ...
  6  | 
  7  |     await page.locator('#sum1').fill('12');
  8  |     await page.locator('#sum2').fill('12');
  9  | 
  10 |     console.log("Sum1:", await page.locator('#sum1').inputValue());
  11 |     console.log("Sum2:", await page.locator('#sum2').inputValue());
  12 | 
  13 |     await page.getByRole('button', {name: 'Get Sum'}).click();
  14 | 
  15 |     await expect(page.locator('#addmessage'))
> 16 |         .toHaveText('24');
     |          ^ Error: expect(locator).toHaveText(expected) failed
  17 | 
  18 |     console.log(
  19 |         "Result:",
  20 |         await page.locator('#addmessage').textContent()
  21 |     );
  22 | 
  23 | });
```