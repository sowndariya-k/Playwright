...
```ts
  ...
  8  | 
  9  | 
  10 |     const messageInput = page.locator("#user-message").first();
  11 | 
  12 | 
  13 |     await expect(messageInput)
  14 |         .toHaveAttribute(
  15 |             "placeholder",
  16 |             "Please enter your Message"
  17 |         );
  18 | 
  19 | 
  20 |     await messageInput.fill("hello");
  21 | 
  22 | 
  23 |     await page.locator("#showInput").click();
  24 | 
  25 | 
  26 |     await expect(page.locator("#message"))
> 27 |         .toHaveText("hello");
     |          ^ Error: expect(locator).toHaveText(expected) failed
  28 | 
  29 | 
  30 |     console.log(
  31 |         "Displayed Message:",
  32 |         await page.locator("#message").textContent()
  33 |     );
  34 | 
  35 | });
```