  ...
  - generic [ref=e2]:
    - generic [ref=e3]:
      - link [ref=e4] [cursor=pointer]:
        - /url: /dashboard.xhtml;jsessionid=node03i6m3m4c6v5v16zieoasu8yx019727943.node0
      - text: 
...
```ts
  ...
  6  | 
  7  |     const chrome = page.getByText("Chrome", { exact: true }).first();
  8  | 
  9  |     await chrome.click();
  10 | 
  11 |     await expect(
  12 |         page.locator(".ui-radiobutton-box").first()
> 13 |     ).toHaveClass(/ui-state-active/);
     |       ^ Error: expect(locator).toHaveClass(expected) failed
  14 | 
  15 | });
```