...
- insertion:
  - iframe
- img
- link "Go to shopping options for Development Tools": Development Tools
...
```ts
  ...
  22 | await page.locator('input[data-qa="submit-button"]').click({ force: true });
  23 | 
  24 | await page.waitForTimeout(5000);
  25 | 
  26 | await expect(page.locator('.status'))
> 27 |   .toContainText('Success! Your details have been submitted successfully.');
     |    ^ Error: expect(locator).toContainText(expected) failed
  28 | 
  29 |   await page.getByRole('link', { name: ' Home' }).click();
  30 | 
