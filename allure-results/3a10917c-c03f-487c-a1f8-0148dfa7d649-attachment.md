...
```
Error: locator.click: Error: strict mode violation: getByLabel('Chrome') resolved to 2 elements:
    1) <input value="" type="radio" id="j_idt87:console1:0" name="j_idt87:console1"/> aka getByRole('radio', { name: 'Chrome' }).first()
    2) <input type="radio" value="Option1" id="j_idt87:console2:0" name="j_idt87:console2"/> aka getByRole('radio', { name: 'Chrome' }).nth(1)

Call log:
  - waiting for getByLabel('Chrome')
  ...
  - generic [ref=e2]:
    - generic [ref=e3]:
      - link [ref=e4] [cursor=pointer]:
        - /url: /dashboard.xhtml;jsessionid=node0vma0c3n2ok2iv4xueh75l8vk19727993.node0
      - text: 
...
```ts
  ...
  6  | 
  7  |     // Click Chrome inside the exercise section
> 8  |     await page.getByLabel("Chrome").click();
     |                                     ^ Error: locator.click: Error: strict mode violation: getByLabel('Chrome') resolved to 2 elements:
  9  | 
  10 |     // Verify Chrome is selected
  11 |     await expect(page.getByLabel("Chrome")).toBeChecked();
  12 | 
  13 | });
```