...
- Name: task\demo.test.ts >> Show Input Message
- Location: tests\task\demo.test.ts:3:1
...
Error: strict mode violation: locator('#user-message') resolved to 3 elements:
...
```ts
  ...
  2  | 
  3  | test("Show Input Message", async ({ page }) => {
  4  | 
  5  |     await page.goto(
  6  |         "https://www.lambdatest.com/selenium-playground/simple-form-demo"
  7  |     );
  8  | 
  9  | 
  10 |     const messageInput = page.locator("#user-message");
  11 | 
  12 |     await expect(messageInput)
> 13 |         .toHaveAttribute(
     |          ^ Error: expect(locator).toHaveAttribute(expected) failed
  14 |             "placeholder",
  15 |             "Please enter your Message"
  16 |         );
  17 | 
  18 | 
  19 |     await messageInput.fill("hello");
  20 | 
  21 | 
  22 |     await page.locator("#showInput").click();
  23 | 
  24 | 
  25 |     await expect(page.locator("#message"))
  26 |         .toHaveText("hello");
  27 | 
  28 | 
  29 |     console.log(
  30 |         "Displayed Message:",
  31 |         await page.locator("#message").textContent()
  32 |     );
  33 | 
  34 | });
```