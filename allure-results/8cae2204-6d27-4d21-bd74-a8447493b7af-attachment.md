...
- Name: task\task2.test.ts >> Sum Two Values
...
```ts
  ...
  2  | 
  3  | test("Sum Two Values", async ({ page }) => {
  4  | 
  5  |     await page.goto(
  ...
  14 | 
  15 | 
  16 |     await page.locator("//button[text()='Get Sum']").click();
  17 | 
  18 | 
  19 |     await expect(page.locator("#addmessage"))
> 20 |         .toHaveText("24");
     |          ^ Error: expect(locator).toHaveText(expected) failed
  21 | 
  22 | 
  23 |     console.log(
  24 |         "Result:",
  25 |         await page.locator("#addmessage").textContent()
  26 |     );
  27 | 
  28 | });
```